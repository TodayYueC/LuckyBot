...
const titles = {
  ...
  settings: ["连接与设置", "把 Unlucky，带到你们的聊天里。"],
...
function shell(content) {
  return `<aside><a class="brand" href="#"><span class="logo">U</span><div>Unlucky<span>群友工作室</span></div></a><div class="side-label">WORKSPACE</div><nav>${Object.entries(
    titles,
  )
    .map(
      ([k, v], i) =>
...
function decisionRows(limit = 30) {
  ...
  return decisions.length
    ? `<div class="decision-list">${decisions
        ...
        .join("")}</div>`
    : '<div class="empty compact">还没有决策记录。到会话空间说声「Unlucky」吧。</div>';
}
...
function chatMessagesMarkup() {
  return (
    messages
      ...
      .join("") ||
    '<div class="empty"><span>☁</span><b>聊天，从一句话开始</b><p>试着分享今天的小事，或者叫一声 Unlucky。</p></div>'
  );
}
...
function settings() {
  ...
  <section class="card form-card"><div class="card-head"><h3>运行与记忆</h3></div>${[
    ["enabled", "允许 Unlucky 参与聊天", "总开关，关闭后所有会话停止回应"],
  ...
  const status = !q.root
    ...
    : install?.error
      ...
      : install?.configDirs?.length
        ...
        : "没有找到 NapCat 配置目录";
  return `<section class="card qq-assistant"><div class="card-head"><div><h3>QQ 接入助手 <small>LOCAL QQ BRIDGE</small></h3><p>Unlucky 自动写入 OneBot 反向连接并启动 NapCat；QQ 登录仍由 QQ 自己扫码确认。</p></div>${badge(state.connection.online ? "QQ 已连接" : "等待 QQ", state.connection.online ? "green" : "")}</div>
...
function render() {
  ...
  if (tab === "persona") {
    ...
    if (hint)
      hint.textContent =
        "每条消息都会先交给语境判断。判断为值得插话时直接回复；只有判断为旁听时才使用这个比例，抽中就会开口。被 @、叫名字、私聊和明确接续 Unlucky 的消息不走这次抽样。";
...
function bindImprovements() {
  ...
  if ($("#launchNapcat"))
    $("#launchNapcat").onclick = async () => {
      ...
      try {
        ...
        toast("已打开 Unlucky NapCat 窗口，请在里面扫码登录 QQ");
      } catch (error) {
        toast(error.message);
      } finally {
        button.disabled = false;
  ...
  if ($("#sessionPolicy"))
    $("#sessionPolicy").onclick = () => {
      ...
      modal(
        "这个会话的聊天节奏",
        `<label>会话名称<input name="name" value="${esc(s.name)}" required maxlength="100"></label><div class="notice">当前生效：普通消息约 ${Math.round(effectiveProbability * 100)}%。留空跟随全局设置；设为 0% 时，仅响应 @、昵称、私聊，以及判断为接在 Unlucky 后面的回应。</div><label>普通消息参与概率（%）<input name="probability" type="number" min="0" max="100" step="1" value="${s.probability == null ? "" : Math.round(s.probability * 100)}" placeholder="全局 ${Math.round(state.settings.probability * 100)}%"></label><label>发言冷却（秒）<input name="cooldown" type="number" min="0" max="3600" value="${s.cooldown ?? ""}" placeholder="全局 ${state.settings.cooldown} 秒"></label>`,
        (v) =>
          api(
