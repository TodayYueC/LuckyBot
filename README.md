...
新增与对话、记忆同级的「时间」工作台：感知聊天间隔、话题随时间降温、低频后台回想，并持续保存可修正的内心手记与状态轨迹。后台独处和主动交流分别开启，按会话授权；容量默认不限，也可自行设置调用、Token、休息时段和防追发边界。[完整使用说明](docs/时间系统.md)。

`love` 分支进一步加入「自己的线索」：LuckyBot 可以从真实聊天中留下自己的好奇、看法和待续方向，之后修正或放下；它们与用户事实记忆分开，按会话隔离，并在后续对话中提供可回看的连续性。认真询问关系时，回复会避开机械的身份免责声明和空泛的情感保证。[设计与验收报告](docs/love-系统报告.md)。
