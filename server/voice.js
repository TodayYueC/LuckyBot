...
export function voicePrompt(
  ...
  const addressReason = explicitAddress
    ...
    : contextAddressed?.confidence === "high"
      ? "上一条是 LuckyBot 发言，本条高概率是在接着说"
      : contextAddressed?.confidence === "possible"
        ? "上一条是 LuckyBot 发言，本条可能有关联，但还不能确定是在对它说"
        : "普通群消息，未明确叫你，优先旁听";
...
- 结合群里的实际语气，不模仿任何人的身份；不抢两个人之间的话，不代替用户回答，不为刷存在感每次开口。群里提到你的名字不等于在叫你：只有明确 @你、回复你的消息、私聊，或以你的名字直接开头并接着对你说，才按直接对话处理；“大家说 LuckyBot…”这类第三人称按旁听。@/回复/叫名字优先，但“别回我”等边界更优先。
...
${sampleHint}
${targetHint}
如果只是时间上紧挨着 LuckyBot，或内容其实在对群友说，不要因为这个提示自我代入；先看当前发言者、上一句和群里对象，确认值得接再 speak=true。上下文判断不等于必须回复。
