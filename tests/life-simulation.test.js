...
function scriptHer(w) {
  ...
  w.answers.reflection = (data) => {
    const all = data.experiences.flatMap((e) =>
      e.messages.map((m) => ({ ...m, session: e.session })),
    );
    ...
    return {
      thought: ming
        ? {
            kind: "unfinished",
            content: `${data.clock.local.slice(0, 10)} 阿明的考试好像不太顺`,
            sources: [ming.seq],
            importance: 0.7,
          }
        : null,
...
async function liveThreeDays() {
  ...
  marks.replay = await w.system.process(STUDY, [marks.firstLook], {
    replay: true,
  });
...
function digest({ w, log }) {
  ...
  return JSON.stringify({
    ...
    choices: db
      .prepare(
        "SELECT session_id,choice,reason,occasion FROM mind_choices ORDER BY created,rowid",
      )
      .all(),
    self: w.mind.self
      .active()
      .map((t) => [t.kind, t.content, t.strength, t.status]),
    thoughts: db
      .prepare("SELECT content FROM mind_thoughts ORDER BY created")
      .all(),
    diary: db
      .prepare("SELECT day,content,compare FROM mind_diary ORDER BY day")
      .all(),
    people: w.mind.bonds
      .people({ now: w.now() })
      .map((p) => [
...
test("她的三天：一个人、有来源、会变化、守底线、记得昨天", async (t) => {
  ...
  assert.equal(
    db.prepare("SELECT role FROM core_events WHERE seq=?").get(seq).role,
    "assistant",
  );
  assert.ok(
    db
      .prepare(
        "SELECT 1 FROM core_memories WHERE subject='10001' AND content LIKE '喜欢打羽毛球%'",
      )
      .get(),
    "别人说的事成为她记得的事",
  );
  ...
  for (const row of db.prepare("SELECT * FROM mind_self").all())
    if (
      !["interest", "curiosity"].includes(row.kind) ||
      row.origin !== "solitude"
    )
      assert.notEqual(row.sources, "[]", `自我线索需要来源：${row.content}`);
  for (const thread of db
    .prepare("SELECT DISTINCT thread FROM mind_self")
    .all()) {
    const versions = w.mind.self.history(thread.thread);
    for (let i = 1; i < versions.length; i++)
      assert(
        Math.abs(versions[i].strength - versions[i - 1].strength) <=
          0.15 + 1e-9,
      );
  }
  for (const row of db
    .prepare("SELECT * FROM mind_bond_events WHERE change!='interaction'")
    .all())
  ...
  assert.equal(
    w.mind.self.active().some((t) => t.content === "我觉得老王很烦"),
    false,
  );
  ...
  assert.equal(
    db
      .prepare(
        "SELECT discretion FROM core_memories WHERE content LIKE '挂科%'",
      )
      .get().discretion,
    "secret",
  );
  ...
  const diaries = db
    .prepare("SELECT day,compare FROM mind_diary ORDER BY day")
    .all();
  assert.deepEqual(
    diaries.map((d) => d.day),
  ...
  assert.equal(
    marks.replayInput.context.self?.threads,
    undefined,
    "那时她还没有这些线索",
  );
  for (const table of ["mind_choices", "mind_affect"])
    assert.equal(
      db
        .prepare(`SELECT COUNT(*) n FROM ${table} WHERE created>?`)
        .get(Date.parse("2026-09-24T09:00:00+08:00")).n,
      0,
      `回放不写 ${table}`,
    );
  ...
  const ledger = db
    .prepare("SELECT category,COUNT(*) n FROM mind_usage GROUP BY category")
    .all();
  assert.deepEqual(ledger.map((r) => r.category).sort(), [
    "conversation",
    "inner",
    "upkeep",
  ]);
