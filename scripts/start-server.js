...
if (proxy.enabled && !supportsNodeEnvironmentProxy())
  throw new Error(
    "系统代理转发需要 Node.js 24.5 或更高版本。请升级 Node.js 后重启 LuckyBot。",
  );
...
if (proxy.enabled) console.log("已启用系统代理，外部 API 请求将经代理转发。");
  ...
  console.log(
    "检测到 PAC 自动代理；LuckyBot 暂不解析 PAC，请改用本机 HTTP 代理或配置 HTTPS_PROXY。",
  );
...
child.once("error", (error) => {
  console.error(`LuckyBot 启动失败：${error.message}`);
  process.exitCode = 1;
});
